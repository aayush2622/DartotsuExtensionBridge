...
import com.lagradost.cloudstream3.RequestBodyTypes
...
open class Slmaxed : ExtractorApi() {
    ...
    override suspend fun getUrl(url: String, referer: String?): List<ExtractorLink>? {
        ...
        val json = app.post(
            ...
            requestBody = """{"id":"$id"}""".toRequestBody("application/json".toMediaTypeOrNull())
        ).parsed<JsonResponse>()
        
        val links = mutableListOf<ExtractorLink>()
        json?.result?.forEach { (_, result) ->
            links.add(
                newExtractorLink(
                    this.name,
                    this.name,
                    result.file ?: return@forEach,
