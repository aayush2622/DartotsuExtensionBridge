...
/// Self-contained CSS-selector support for the `html` package, used by
/// LNReader's cheerio polyfill ([Cheerio]). Ported from Mangayomi's
/// `dom_extensions.dart` but trimmed to just `select` / `selectFirst` plus the
...
extension LnDocumentSelector on Document? {
...
extension LnElementSelector on Element {
  List<Element>? select(String selector) {
    try {
      _ensurePseudo();
