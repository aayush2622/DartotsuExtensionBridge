...
object ShortLink {
    ...
    suspend fun unshortenLinkup(uri: String): String {
        ...
        if (r == null) {
            r = app.get(uri)
            val location = r.okhttpResponse.headers["location"]
            if (location != null) {
                uri = location
            }
        }
    ...
    suspend fun unshortenNuovoIndirizzo(uri: String): String {
        val soup = app.get(uri)
        val header = soup.okhttpResponse.headers["refresh"]
    ...
    suspend fun unshortenNuovoLink(uri: String): String {
        return app.get(uri).document.selectFirst("a")!!.attr("href")
