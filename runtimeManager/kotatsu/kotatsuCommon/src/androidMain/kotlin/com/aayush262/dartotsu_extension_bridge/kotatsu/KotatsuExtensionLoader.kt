...
actual object KotatsuExtensionLoader {
    ...
    actual suspend fun loadExtensions(folderPath: String?): List<Map<String, Any?>> {
        ...
        return scanMutex.withLock {
            ...
            if (cacheFile.exists()) {
                try {
                    ...
                    val jsonArray = JSONArray(cachedText)
                        ...
                        try {
                            ...
                            if (isMangaParser &&
                                ...
                                val rawParser = try {
                                    ...
                                    val realException = if (e is InvocationTargetException) e.targetException else e
...
class KotatsuMangaParserWrapper(
    ...
    override fun intercept(chain: Interceptor.Chain): Response {
        return try {
            val interceptMethod = rawParser.javaClass.getMethod("intercept", Interceptor.Chain::class.java)
            interceptMethod.invoke(rawParser, chain) as Response
        } catch (e: Exception) {
    ...
    private suspend fun invokeSuspendMethod(method: Method, target: Any, vararg args: Any?): Any? {
